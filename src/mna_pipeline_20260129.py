...
def run_event_study(panel_df, deals_df, min_deal_value=100):
    ...
    try:
        ...
        if jump > 0.15:
            ...
            log("⚠️ VERDICT: Signal muted. Check for rumor leak or deal mix.")
    except Exception as e:
        log(f"[WARN] Could not compute jump: {e}")

# Run Event Study
run_event_study(labeled_panel, deals_df)
log("✅ Cell H Complete: Event Study.")
...
log("✅ Cell I Complete: Correlation Screening Functions Defined.")
...
log("✅ Cell J Complete: Quarter Report Functions Defined.")
...
def create_network_gif(panel_df, deals_df, quarters, target_col, output_path, fps=1):
    ...
    log(f"✅ Network GIF saved to {output_path}")
...
log("✅ Cell K Complete: Network GIF.")
...
if "labeled_panel" in dir() and "final_feats" in dir():
    ...
    if adv_results is not None:
        ...
        log(f"✅ Advanced Backtest saved to {adv_path}")
...
log("✅ Pipeline Complete with All Features.")
...
def calibrate_quarter(panel_df, train_df, test_df, target_col, model, feature_cols, 
    ...
    if pi_train_sample != pi_train_full:
        ...
        log(f"Prior correction applied: π_full={pi_train_full:.4f}, π_sample={pi_train_sample:.4f}")
    else:
    ...
    print(f"{'='*50}")
    print(f"π_train_full (true base rate):    {pi_train_full:.4f}")
    print(f"π_train_sample (after downsample): {pi_train_sample:.4f}")
    print(f"π_calibration_set:                 {pi_cal:.4f}")
    print(f"π_holdout:                         {y_test.mean():.4f}")
    print(f"\nBrier Scores:")
...
log("✅ Cell M Complete: Probability Calibration Pipeline Defined.")
...
def run_calibrated_backtest(panel_df, target_col, feature_cols, artifact_dir):
    ...
    log(f"✅ Calibrated Backtest saved to {results_path}")
    log(f"✅ Calibration Summary saved to {cal_summary_path}")
...
log("✅ Cell N Complete: Calibrated Backtest.")
...
print("  ✅ Multi-horizon labeling (3m-24m)")
print("  ✅ S&P 500 baseline comparison")
print("  ✅ Stability dry-run harness (Jaccard overlap)")
print("  ✅ Event study verification (jump plot)")
print("  ✅ Correlation screening for portfolio construction")
print("  ✅ Network correlation GIF")
print("  ✅ Quarter deep-dive reports")
print("  ✅ Probability calibration (prior correction + isotonic/Platt)")
